...
class BookRepositoryTest {
    ...
    void testSavingABook() {
        ...
        RepositoriesTestHelper.assertSameBook(
            savedBook,
            retrievedBook,
    ...
    private <C> void retrieveTesting(
        ...
        final var books = TestHelper.mockBookList(maxListSize);
        final var book = books.get(0); // There is always at least one.
        books.forEach(entityManager::persist);
        entityManager.flush();
        final var condition = conditionGetter.apply(book);
        retrievingOperation.apply(bookRepository, condition);
        final var retrievedBookOptional = bookRepository.findById(book.getId());
        Assertions.assertTrue(
            retrievedBookOptional.isPresent(),
            "The retrieving operation did not retrieve an existing book"
        );
        final var retrievedBook = retrievedBookOptional.get();
        RepositoriesTestHelper.assertSameBook(
            book,
            retrievedBook,
            "Another book has been retrieved"
        );
    }
}
